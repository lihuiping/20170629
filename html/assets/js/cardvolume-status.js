...
var client2 = hprose.Client.create("http://test.7cai.tv/index.php/api/api/activity?t=" + token, ["getVoucherLists"]);
client2.getVoucherLists({
	"type": 1
}, function(result) {
	var result = $.parseJSON(result);
	Boolean(result.data) == true;
	if(result.data == false) {
		$('#zx-tab1 .content-block').show();
		$('.wsy_kaquan').hide();
	} else {
		$('#zx-tab1 .content-block').hide();
		$('.wsy_kaquan').show();
	};
});
